...
/**
 * Hook to get layout options from config
 */
...
/**
 * Hook to get Ralph loop options from config
 */
export function useRalphLoopConfig(): {
  options: ReturnType<typeof getRalphLoopOptions>;
  isLoading: boolean;
} {
  const { config, isLoading } = useConfig();
  return {
    options: getRalphLoopOptions(config),
    isLoading,
  };
}

/**
 * Hook to get theme options from config
 */
export function useThemeConfig(): {
  options: ReturnType<typeof getThemeOptions>;
  isLoading: boolean;
} {
  const { config, isLoading } = useConfig();
  return {
    options: getThemeOptions(config),
    isLoading,
  };
}
